...
/** Grounded AI Q&A widget — answers come from the same timetable engine as the
 * search, via /api/trains/assistant. */
export default function TrainAssistant() {
  ...
  const ask = async (question: string) => {
    ...
    setInput('');
    setLoading(true);
    trackEvent('train_assistant_ask');
    const nextMessages: Message[] = [...messages, { sender: 'user', text: q }];
    setMessages(nextMessages);
    try {
      const res = await fetch('/api/trains/assistant', {
        method: 'POST',
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 md:p-8 space-y-5">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-white">AI Rail Assistant</h2>
          <p className="text-[11px] text-neutral-500">Answers grounded in our timetable database — not guesswork.</p>
        </div>
      </div>
        ...
        <div ref={scrollRef} className="max-h-80 overflow-y-auto space-y-3 pr-1" aria-live="polite">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs text-neutral-500 p-2">
              <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden /> Checking timetables…
            </div>
          )}
        </div>
      )}
      ...
      <p className="text-[10px] text-neutral-600 leading-relaxed">
        Schedules and fares quoted by the assistant are indicative. Our rail desk confirms live availability and pricing before any booking.
      </p>
    </div>
  );
}
