...
import type { CarReview } from '@/lib/cars/types';

const PAGE_SIZE = 3;

export default function CompanyReviews({ companyName, reviews }: { companyName: string; reviews: CarReview[] }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm space-y-6">
      <div>
        <h4 className="font-bold text-slate-900 dark:text-slate-50">Renter Reviews — {companyName}</h4>
        <p className="text-xs text-slate-500">Feedback from verified rentals with this company.</p>
      </div>
      ...
      <div className="space-y-4 divide-y divide-slate-100 dark:divide-slate-800 max-h-[600px] overflow-y-auto">
        {visible.map((r, idx) => (
          <div key={r.id} className={`space-y-2 ${idx > 0 ? 'pt-4' : ''}`}>
            ...
            <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">{r.title}</p>
