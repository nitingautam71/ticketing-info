...
import { Search, Compass, ShieldCheck, ShieldAlert, Check, ChevronRight, Calculator } from 'lucide-react';
...
export default function InsuranceTab({ onSelectBookingItem }: InsuranceTabProps) {
  const [plans, setPlans] = useState<InsurancePlan[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [hasSearched, setHasSearched] = useState(false);
  const [tripCost, setTripCost] = useState<number>(2000);
  ...
  const triggerCheckout = (plan: InsurancePlan) => {
    onSelectBookingItem({
      type: 'insurance',
      title: plan.name,
      ...
      details: {
        planName: plan.name,
        tier: plan.tier,
        medicalCoverage: plan.medicalCoverage,
        tripCancellation: plan.tripCancellation,
        luggageCoverage: plan.luggageCoverage
      }
    });
  };
      ...
      <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-neutral-400 max-w-xl">
            Aviation delays, medical issues, or baggage transfers can introduce risks to any itinerary. Protect your investments with standard global protection tiers.
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {plans.map((p) => (
                <div 
                  key={p.id}
                  className="bg-neutral-900 border border-neutral-800 hover:border-emerald-500/30 rounded-2xl p-6 flex flex-col justify-between space-y-6 shadow transition-colors"
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] bg-neutral-950 text-neutral-400 border border-neutral-850 px-2 py-0.5 rounded-full font-mono font-bold uppercase tracking-wider">
                          {p.tier} Tier
                        </span>
                  ...
                  <div className="border-t border-neutral-850 pt-4 flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs px-4 py-2.5 rounded-xl font-bold flex items-center gap-1 cursor-pointer transition-colors"
                    >
                      Instant Purchase <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ))}
