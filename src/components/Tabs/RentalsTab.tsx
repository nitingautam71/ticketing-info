...
import { Search, Car, Calendar, User, Settings, Star, ChevronRight, AlertCircle } from 'lucide-react';
...
export default function RentalsTab({ onSelectBookingItem }: RentalsTabProps) {
  const [pickup, setPickup] = useState('London Heathrow Airport');
  const [pickupDate, setPickupDate] = useState('2026-08-15');
  const [dropoffDate, setDropoffDate] = useState('2026-08-19');
  ...
  const triggerCheckout = (car: RentalCar) => {
    // Calculate total days (default to 4 days if date parsing has any issues)
    const days = 4;
    onSelectBookingItem({
      type: 'rental',
      ...
      <form onSubmit={handleSearch} className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl shadow-xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          ...
          <div className="md:col-span-1">
            <button
              type="submit"
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl py-3.5 flex items-center justify-center cursor-pointer transition-colors"
            >
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {rentals.map((car) => (
                <div 
                  key={car.id}
                  className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden shadow-lg hover:border-neutral-700 transition-all flex flex-col justify-between"
                >
