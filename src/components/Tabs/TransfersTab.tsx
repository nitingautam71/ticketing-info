...
import { Search, Compass, MapPin, Calendar, Clock, ChevronRight, User, AlertCircle, Info } from 'lucide-react';
...
export default function TransfersTab({ onSelectBookingItem }: TransfersTabProps) {
  ...
  const triggerCheckout = (tf: Transfer) => {
    onSelectBookingItem({
      type: 'transfer',
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {transfers.map((tf) => (
                <div 
                  key={tf.id}
                  className="bg-neutral-900 border border-neutral-800 p-5 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow"
                >
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex gap-4 text-[10px] text-neutral-500 font-medium">
                      <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" /> Accommodates {tf.capacity} guests</span>
