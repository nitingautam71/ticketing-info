...
import { Search, Plane, Calendar, ArrowUpDown, Filter, ChevronRight, User, AlertCircle, Briefcase } from 'lucide-react';
...
export default function FlightSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setHasSearched(true);
    try {
      const res = await fetch(`/api/flights?from=${from}&to=${to}&date=${date}&class=${cabinClass}`);
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {isLoading ? (
              ...
              filteredFlights.map((f) => (
                <div key={f.id} className="bg-neutral-900 border border-neutral-800 hover:border-neutral-700 rounded-2xl overflow-hidden shadow-md transition-all group">
                  <div className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                    ...
                    <div className="flex sm:flex-col justify-between items-end gap-3 w-full sm:w-auto border-t sm:border-t-0 border-neutral-800 pt-4 sm:pt-0 shrink-0">
                      <div className="sm:text-right">
                        ...
                        {passengerCount > 1 && <p className="text-[10px] text-emerald-400 font-mono mt-0.5">Total: ${(f.price * passengerCount).toLocaleString()}</p>}
                      </div>
                      <button
