...
import { Search, MapPin, Calendar, Star, Filter, Coffee, ChevronRight, AlertCircle } from 'lucide-react';
import type { Hotel, HotelRoomType } from '@/lib/providers/hotels';
...
export default function HotelSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/hotels?location=${encodeURIComponent(location)}`);
  ...
  const selectRoom = (hotel: Hotel, room: HotelRoomType) => {
    ...
    open({
      ...
      details: {
        ...
        roomType: room.name,
        checkIn: checkInDate,
        checkOut: new Date(new Date(checkInDate).getTime() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        guests,
      },
    });
  };
          ...
          {!selectedHotel && (
            <div className="bg-neutral-900 border border-neutral-800 p-5 rounded-2xl h-fit space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input type="range" min={100} max={1000} step={20} value={maxPrice} onChange={(e) => setMaxPrice(parseInt(e.target.value))} className="w-full accent-emerald-500" />
              </div>
                ...
                <div className="space-y-2 text-xs">
                  {['all', '5', '4', '3'].map((opt) => (
                    <label key={opt} className="flex items-center gap-2 cursor-pointer text-neutral-350 hover:text-white">
                      <input type="radio" name="stars" checked={starsFilter === opt} onChange={() => setStarsFilter(opt)} className="accent-emerald-500" />
              ...
              <div className="space-y-2">
                ...
                <select value={selectedAmenity} onChange={(e) => setSelectedAmenity(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2 text-xs text-white">
            ...
            {isLoading ? (
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden shadow-xl">
                <div className="h-64 relative bg-neutral-950">
                  <Image src={selectedHotel.image} alt={selectedHotel.name} fill sizes="100vw" className="object-cover opacity-80" />
                      ...
                      {selectedHotel.roomTypes.map((room) => (
                        <div key={room.name} className="bg-neutral-950 border border-neutral-850 rounded-xl p-5 flex flex-col justify-between space-y-4 shadow hover:border-emerald-500/35 transition-colors">
                          ...
                          <div className="border-t border-neutral-850 pt-3 flex justify-between items-end">
                            ...
                            <button onClick={() => selectRoom(selectedHotel, room)} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs px-3.5 py-2 rounded-lg font-bold flex items-center cursor-pointer transition-colors">
              ...
              filteredHotels.map((h) => (
                <div key={h.id} className="bg-neutral-900 border border-neutral-800 hover:border-neutral-700 rounded-2xl overflow-hidden shadow-md flex flex-col sm:flex-row transition-all group">
                  <div className="w-full sm:w-[220px] h-[160px] sm:h-auto relative bg-neutral-950 shrink-0">
                    <Image src={h.image} alt={h.name} fill sizes="(min-width: 640px) 220px, 100vw" className="object-cover opacity-80" />
                  </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                    ...
                    <div className="flex sm:flex-row justify-between items-end border-t border-neutral-800/60 pt-4 mt-2">
                      ...
                      <div className="text-right flex items-center gap-4">
                        ...
                        <button onClick={() => setSelectedHotel(h)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2.5 rounded-xl flex items-center gap-1 cursor-pointer transition-colors">
