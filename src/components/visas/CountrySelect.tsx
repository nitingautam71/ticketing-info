...
import { VISA_COUNTRIES_SORTED, countryByCode, flagEmoji } from '@/lib/visas/countries';

/**
 * Accessible searchable country combobox (ARIA 1.2 combobox pattern): type to
 * filter all 199 countries, arrow keys + Enter to pick, Escape to close.
 */
export default function CountrySelect({
  ...
  const results = useMemo(() => {
    ...
    if (!q) return VISA_COUNTRIES_SORTED;
    return VISA_COUNTRIES_SORTED.filter(
      (c) => c.name.toLowerCase().includes(q) || c.code.toLowerCase() === q || c.slug.includes(q.replace(/\s+/g, '-')),
    );
  }, [query]);
    ...
    <div ref={rootRef} className="relative">
      ...
      <div className="relative">
        <input
          ...
          aria-label={label}
          value={open ? query : selected ? `${flagEmoji(selected.code)} ${selected.name}` : query}
      ...
      {open && (
        ...
        >
          {results.length === 0 && <li className="px-4 py-2.5 text-xs text-neutral-500">No matching country</li>}
          {results.map((c, i) => (
            <li
              key={c.code}
              role="option"
            ...
            >
              ...
              <span>{c.name}</span>
              <span className="ml-auto text-[10px] text-neutral-500 font-mono">{c.code}</span>
            </li>
          ))}
        </ul>
      )}
