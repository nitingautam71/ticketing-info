...
const SUGGESTIONS = [
  'Can I visit Japan with an Indian passport?',
  'Do I need a transit visa connecting through Dubai?',
  'What documents do US citizens need for Brazil?',
  'Can my spouse travel with me on a student visa?',
];

/** Grounded AI Q&A widget — answers come from the same rules engine as the
 * checker, via /api/visa/assistant. */
export default function VisaAssistant() {
  ...
  const ask = async (question: string) => {
    ...
    setInput('');
    setLoading(true);
    trackEvent('visa_assistant_ask');
    const nextMessages: Message[] = [...messages, { sender: 'user', text: q }];
    setMessages(nextMessages);
    try {
      const res = await fetch('/api/visa/assistant', {
        method: 'POST',
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 md:p-8 space-y-5">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-sm font-bold text-white">AI Visa Assistant</h2>
          <p className="text-[11px] text-neutral-500">Answers grounded in our live rules database — not guesswork.</p>
        </div>
      </div>
        ...
        <div ref={scrollRef} className="max-h-80 overflow-y-auto space-y-3 pr-1" aria-live="polite">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs text-neutral-500 p-2">
              <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden /> Checking the rules…
            </div>
          )}
        </div>
      )}
      ...
      >
        <input
          ...
          maxLength={600}
          placeholder="Ask anything — e.g. “Can I extend a Thailand visa on arrival?”"
          aria-label="Ask the visa assistant"
      ...
      <p className="text-[10px] text-neutral-600 leading-relaxed">
        AI answers are guidance, not legal advice. Official requirements are always confirmed by our consultants before any application.
      </p>
    </div>
  );
}
